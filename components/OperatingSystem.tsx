...
export function OperatingSystem({ variant = "full" }: OperatingSystemProps) {
  const phases: Phase[] = [
    {
      number: 1,
  ...
  if (variant === "condensed") {
    ...
    return (
      <div className="grid md:grid-cols-3 gap-6">
        {condensedPhases.map((phase) => (
          <Card key={phase.number} className="p-6">
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      {phases.map((phase, index) => (
        <Card key={phase.number} className="p-6 md:p-8">
